...
from .celligner2_model import CELLIGNER2

class Adaptor:
    """Adaptor class for celligner2.

       Allows to save and load trainded conditional weights for celligner2 models.

       Parameters
       ----------
       celligner2_model
            A CELLIGNER2 class object with a trainded model or a path to saved Adaptor object.
       condition
            Condition name to save in the adaptor.
    """
    model_type = 'celligner2'

    def __init__(
        self,
        celligner2_model: Union[str, CELLIGNER2],
        condition: Optional[str] = None
    ):
        if isinstance(celligner2_model, str):
            cond_params_path = os.path.join(celligner2_model, "cond_params.pt")
            adapt_params_path = os.path.join(celligner2_model, "adapt_params.pkl")
            ...
            cond_idx = celligner2_model.conditions_.index(self.condition)
            for name, p in celligner2_model.model.state_dict().items():
            ...
            self._adapt_params['model_params']['varnames'] = celligner2_model.adata.var_names.tolist()

            self._adapt_params['model_params']['hidden_layer_sizes'] = celligner2_model.hidden_layer_sizes_
            self._adapt_params['model_params']['latent_dim'] = celligner2_model.latent_dim_
            self._adapt_params['model_params']['recon_loss'] = celligner2_model.recon_loss_
...
def attach_adaptors(
    celligner2_model: CELLIGNER2,
    adaptors: list,
    only_new: bool = False
):
    """Attach the conditional weights from the adaptors to a celligner2 model.
       ...
       ----------
       celligner2_model
            A CELLIGNER2 class object. The object should have the same architecture
       ...
       only_new
            ...
            which are already in the model (in `celligner2_model.conditions_`).
    """
    attr_dict = celligner2_model._get_public_attributes()
    init_params = deepcopy(CELLIGNER2._get_init_params_from_dict(attr_dict))
    ...
    for adaptor in adaptors:
        ...
        adaptor._validate_params(celligner2_model.adata.var_names, init_params)
    ...
    init_params['conditions'] += [adpt_csonditions[i] for i in inds_new]

    new_model = CELLIGNER2(celligner2_model.adata, **init_params)
    state_dict = celligner2_model.model.state_dict().copy()
