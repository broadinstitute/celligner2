...
def label_encoder(adata, encoder, label_key=None):
    ...
    if not set(unique_conditions).issubset(set(encoder.keys())):
        print(
            f"Warning: Labels: {set(unique_conditions)-set(encoder.keys())} in \
...
def label_encoder_2D(adata, encoder, label_sets):
    ...
    encoder: Dict
         dictionary of encoded labels.
    label_sets: Dict
         {column name of conditions in `adata.obs` data frame: list[set of conditions in each columns]}
